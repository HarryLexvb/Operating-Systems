...
void func(pid_t child_pid, int n_bytes, char *read_buffer, int *fd, int *fd1){
    if((child_pid = fork()) == -1) {    // Create a child process
        ...
        mtx.lock();
        // Create a fifo
        char *fifo = ((char*)"/home/harrylex/Documentos/Operating Systems/control1/control V3/Mymkfifo/fifo");
...
int main(){
    ...
    // Create a pipe
    pipe(fd1);
    int n_bytes = 0; // Number of bytes read from the pipe
    pid_t child_pid = 0; // Child process id
    char *read_buffer = new char[100]; // Buffer to read from the pipe
    func(child_pid, n_bytes, read_buffer, fd, fd1); // Call the function func
}
